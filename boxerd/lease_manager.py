...
class LeaseManager:
        ...
        for vm in vms:
            ...
            if vm.state == "running":
                ...
                if vm.lease_until < delete_threshold:
                    ...
                    await self._auto_delete(vm.id, vm.libvirt_name, vm.project_id)
    ...
    async def _auto_delete(self, vm_id: str, libvirt_name: str, project_id: str) -> None:
        try:
            self._vm_ops.undefine(libvirt_name)
            self._storage.delete_vm_storage(project_id, vm_id)
            await self._db.delete_vm(vm_id)
            await self._db.add_event(
                "INFO",
                f"Auto-deleted stale VM {vm_id} ({libvirt_name})",
                project_id=project_id,
                vm_id=vm_id,
            )
        except Exception as exc:
            logger.exception("Failed to auto-delete VM %s: %s", vm_id, exc)
